...
def prepare_atmosphere(atm_ds: xr.Dataset, species: tuple[str, ...]) -> xr.Dataset:
    ...
    atm_ds = atm_ds.rename(species_rename)  # .isel(variant=0)
...
def _add_global_fluxes_if_available(
    fluxes: xr.Dataset, atm_ds: xr.Dataset, band: str
) -> None:
...
def do_arts_ddq_example_variant(
    atm_ds: xr.Dataset,  # already isel'd to one variant, NOT yet loaded
    band: str,
...
#     variant_fluxes = []
#     for v in range(atm_lazy.sizes["variant"]):
#         atm_v = atm_lazy.isel(variant=v)
...
#         f = xr.merge([lw, sw], compat="equals", join="outer")
#         add_net_flux(f)
#         variant_fluxes.append(f)
#     fluxes = xr.concat(variant_fluxes, dim="variant")

#     add_net_flux(fluxes)

#     case = file_path.name.split("-")[0]
#     output_path = output_dir / f"pyarts_ddq_fluxes_{case}2.nc"
